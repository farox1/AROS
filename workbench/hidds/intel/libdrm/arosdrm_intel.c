    ...
    unsigned long size, const void *data)
{
    ...
    {
        D(bug("[AROS libdrm] bo_subdata: OOB offset=%lu size=%lu bosize=%lu\n",
            offset, size, bo->size));
        return -1;
    }
    ...
    unsigned long size, void *data)
{
    ...
    if (offset > bo->size || size > bo->size - offset)
    {
        D(bug("[AROS libdrm] bo_get_subdata: OOB offset=%lu size=%lu bosize=%lu\n",
            offset, size, bo->size));
        return -1;
    }
...
int drm_intel_gem_bo_map_gtt(drm_intel_bo *bo)
{
    ...
    /*
     * The AROS DRM layer stores the CPU address of the GEM backing store in
     * mmap_gtt.offset (see DRM_IOCTL_I915_GEM_MMAP_GTT in arosdrm.c). Map bo->virtual
     * to that same memory instead of allocating a second, disconnected buffer.
     * A separate AllocVec here made bo->virtual diverge from the buffer used by
     * the pwrite/pread ioctls, and those two live buffers leaked on every
     * map/unmap cycle, which is what corrupted the heap on the CPU-fallback path.
     */
    bo->virtual = (void *)(unsigned long)mmap_gtt.offset;
    if (!bo->virtual)
    {
        D(bug("[AROS libdrm] MMAP_GTT returned NULL address\n"));
        return -1;
    }

    D(bug("[AROS libdrm] map_gtt: handle=%u -> virtual=%p (size=%lu)\n",
        bo->handle, bo->virtual, bo->size));
