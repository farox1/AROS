...
#define GTT_FB_ADDR 0xd0000000UL
...
enum
{
    aoHidd_Gfx_Intel_CompositorClass,           /* [I..] Per-driver compositor class override */

    num_Hidd_Gfx_Intel_Attrs
};

#define aHidd_Gfx_Intel_CompositorClass    (HiddGfxIntelAttrBase + aoHidd_Gfx_Intel_CompositorClass)

#define IS_GFXINTEL_ATTR(attr, idx) \
    (((idx) = (attr) - HiddGfxIntelAttrBase) < num_Hidd_Gfx_Intel_Attrs)
...
struct HIDDIntelData
{
    ...
    OOP_Object          *framebuffer;           /* The screen framebuffer bitmap */
};
...
struct HIDDIntelBitMapData
{
    ...
    BOOL    is_framebuffer; /* Uses GTT aperture, not GEM buffer */
    APTR    gtt_ptr;        /* Direct GTT pointer for framebuffer */
...
struct CardData
{
    ...
    APTR                    mmio;                    /* Mapped MMIO registers */
    ULONG                   mmio_size;               /* MMIO region size */

    APTR                    gtt_fb;                  /* GTT aperture framebuffer address */
    ULONG                   gtt_fb_pitch;            /* Framebuffer stride */

    ULONG                   mode_width;              /* Currently displayed mode */
    ULONG                   mode_height;             /* Currently displayed mode */
    ULONG                   mode_bpp;                /* Currently displayed bit depth (16/24/32) */
    APTR                    selected_connector;      /* Selected drmModeConnector */

    ULONG                   panel_width;             /* Panel native width (scaler output) */
    ULONG                   panel_height;            /* Panel native height (scaler output) */
    BOOL                    is_edp;                  /* TRUE if internal eDP panel (TRANSCODER_EDP) */

    /* Display topology detected from the BIOS state. Used to make the DP path
     * target the actual active port/transcoder/pipe instead of hardcoding
     * port E + transcoder A + pipe A (which only works on that one config). */
    ULONG                   display_port;            /* Active DDI port 0=A..4=E, or 0xFFFFFFFF */
    ULONG                   display_trans;           /* Active transcoder 0=A..3=D, or 0xFFFFFFFF */
    ULONG                   display_pipe;            /* Active pipe 0=A..2=C, or 0xFFFFFFFF */
    ULONG                   display_aux;             /* Active DP AUX channel 0=A..4=E, or 0xFFFFFFFF */
    BOOL                    is_icl_plus;             /* TRUE for Gen 11+ (Ice Lake/Tiger Lake) */
    BOOL                    is_tgl_plus;             /* TRUE for Gen 12+ (Tiger Lake): DDI port field moved */

    /* Active output type, decoded at probe from the TRANS_DDI_FUNC_CTL mode
     * select. Drives which enable sequence set_mode uses. */
    enum
    {
        INTEL_OUTPUT_UNKNOWN = 0,
        INTEL_OUTPUT_DP      = 1,
        INTEL_OUTPUT_EDP     = 2,
        INTEL_OUTPUT_HDMI    = 3,
        INTEL_OUTPUT_DVI     = 4,
        INTEL_OUTPUT_LEGACY  = 5,   /* pre-DDI (Sandy/Ivy): FDI/PCH, re-show only */
    }                       output_type;

    /* HDMI clone (laptop DDI B): bring up a second output mirroring the active
     * mode on the internal panel. Used as the laptop-HDMI bring-up path. */
    BOOL                    hdmi_clone_enable;
    BOOL                    hdmi_clone_done;
    ULONG                   hdmi_clone_clock;    /* hz programmed into DPLL1 */

    struct pci_dev           *pdev;                   /* PCI device for config access */
};
...
#define MAP_BUFFER                  { if (!bmdata->is_framebuffer && !bmdata->bo->virtual) { drm_intel_gem_bo_map_gtt(bmdata->bo); } }
#define BM_VIRTUAL(bmdata)          (bmdata->is_framebuffer ? bmdata->gtt_ptr : bmdata->bo->virtual)
...
/* GPU MMIO register access helpers */
#define INTEL_READ(carddata, reg)   readl((APTR)((IPTR)(carddata)->mmio + (reg)))
#define INTEL_WRITE(carddata, reg, val) writel((val), (APTR)((IPTR)(carddata)->mmio + (reg)))
...
/* Intel display mode setting (intel_modeset.c) */
/* bpp = bit depth of the framebuffer (16, 24, 32) -> selects the PLANE pixel format */
BOOL intel_set_mode(struct CardData *carddata, APTR mode, ULONG pitch, ULONG bpp);
BOOL intel_set_mode_by_resolution(struct CardData *carddata, APTR connector, ULONG width, ULONG height, ULONG pitch, ULONG bpp);
BOOL intel_mode_lookup_and_set(struct CardData *carddata, APTR connector, ULONG modeid, ULONG bytesperpixel, ULONG *width, ULONG *height, ULONG *pitch);

/* Read the monitor/panel EDID over AUX and return the preferred DTD (intel_modeset.c) */
BOOL intel_read_edid_preferred(struct CardData *carddata, APTR mode);

/* Global carddata pointer for xf86-video-intel compatibility */
extern struct CardData * globalcarddataptr;

/* PCI DRM initialization */
extern int intel_drm_pci_init(void);
