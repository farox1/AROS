...
static void worker_main(void)
{
    ...
    D(bug("[Intel] WorkQueue worker started\n"));

    workqueue_port = CreateMsgPort();
    if (!workqueue_port)
    {
        D(bug("[Intel] WorkQueue: Failed to create message port\n"));
        return;
    }

    D(bug("[Intel] WorkQueue: Message port created, signaling parent\n"));
    ...
    for (;;)
    {
        ...
        {
            D(bug("[Intel] WorkQueue: Executing work %p func %p\n", wmsg->work, wmsg->work->func));
...
static void timer_main(void)
{
    ...
    D(bug("[Intel] TimerProcess started\n"));

    timer_port = CreateMsgPort();
    if (!timer_port)
    {
        D(bug("[Intel] TimerProcess: Failed to create message port\n"));
        return;
    }

    D(bug("[Intel] TimerProcess: Message port created, signaling parent\n"));
    ...
    for (;;)
    {
        ...
        while ((msg = GetMsg(timer_port)))
        {
            ...
            D(bug("[Intel] TimerProcess: Timer fired for dwork %p\n", dwork));
...
BOOL workqueue_init()
{
    D(bug("[Intel] WorkQueue: Initializing\n"));
    ...
    if (!workqueue_proc)
    {
        D(bug("[Intel] WorkQueue: Failed to create worker process\n"));
        parent_task = NULL;
        return FALSE;
    }
    ...
    if (!timer_proc)
    {
        D(bug("[Intel] WorkQueue: Failed to create timer process\n"));
        return FALSE;
    }

    Wait(SIGF_SINGLE);

    D(bug("[Intel] WorkQueue: Initialized successfully\n"));
    return TRUE;
}
...
bool queue_work(struct workqueue_struct *wq, struct work_struct *work)
{
    ...
    if (!wmsg)
    {
        D(bug("[Intel] WorkQueue: Failed to allocate message\n"));
        return FALSE;
    }
    ...
    D(bug("[Intel] WorkQueue: Queuing work %p\n", work));
...
{
    D(bug("[Intel] schedule_delayed_work dwork=%p delay=%lu jiffies\n", dwork, delay));
...
bool cancel_delayed_work_sync(struct delayed_work *dwork)
{
    ...
    D(bug("[Intel] cancel_delayed_work_sync dwork=%p\n", dwork));
