...
void __free_page(struct page * p)
{
    if (p->allocated_buffer)
        HIDDIntelFree(p->allocated_buffer);
...
{
    struct page * p;
    p = HIDDIntelAlloc(sizeof(*p));
    p->allocated_buffer = HIDDIntelAlloc(PAGE_SIZE + PAGE_SIZE - 1);
    ...
    size_t num_pages , u32 type)
{
    ...
    struct agp_memory * mem = HIDDIntelAlloc(sizeof(struct agp_memory));
    mem->pages = HIDDIntelAlloc(sizeof(struct page *) * num_pages);
...
struct agp_bridge_data * agp_find_bridge(void * dev)
{
    ...
    if(agpbus)
    {
        ...
        if (bridgedevice)
        {
            ...
            global_agp_bridge = HIDDIntelAlloc(sizeof(struct agp_bridge_data));
